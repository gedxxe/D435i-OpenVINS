...
root README.
It creates an ignored bundle under `config/local/d435i-SERIAL/` because
OpenVINS resolves IMU and camera files relative to the main YAML.
...
The full Allan/Kalibr workflow in the root README creates the only supported
...
The reviewed project patch also enables continuous stop recovery:
...
requires accumulated disparity to remain below `zupt_max_disparity` for the
confirmation interval, and rechecks that evidence while stopped, so slow real
camera motion returns to the normal visual-inertial update path.
