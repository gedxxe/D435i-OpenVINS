...
<p align="center">
  <img src="docs/assets/hero.svg" alt="D435i stereo and IMU feeding standalone OpenVINS and a global XYZ trajectory" width="100%">
</p>
...
> consistency; it is never eliminated by a hidden scale factor.
...
| Default streams | 848×480 @ 30 Hz, gyro 200 Hz, accelerometer 250 Hz |
...
| Output | pose, velocity, biases, covariance, trajectory log, isometric XYZ viewer |
...
<p align="center">
  <img src="docs/assets/runtime-pipeline.svg" alt="OVRS deterministic runtime pipeline" width="100%">
</p>
...
Librealsense supplies motion values in rad/s and m/s². OVRS does not multiply
them by an empirical unit scale. The accelerometer measurement is first
rotated with the device-reported accel-to-gyro rotation:
...
Low feature disparity is an additional gate, never permission to bypass the
IMU innovation or maximum-velocity gates. Missing visual tracks mean
“unknown,” not “stationary.” The shipped default restricts ZUPT to
initialization because continuous ZUPT can falsely suppress deliberate slow
handheld motion.
...
flowchart TD
    ...
    D --> E{"Existing local bundle?"}
    E -- "Contains T_cam_imu" --> F["Migrate once; keep backup"]
    E -- "Contains T_imu_cam" --> G["Validate; do not overwrite"]
    E -- "No" --> H["Inspect and create bootstrap bundle"]
    F --> I{"Goal?"}
    G --> I
    H --> I
    I -- "Capture/replay mechanics only" --> J["Use explicit unverified diagnostic override"]
    I -- "Trustworthy VIO" --> M{"Is a pinned isolated ROS1/Kalibr environment available?"}
    M -- "No" --> V{"Install Docker CE from official Ubuntu repository?"}
    V -- "No" --> X
    V -- "Yes" --> W["Install, reboot/login, verify non-root hello-world"]
    W --> K
    M -- "Yes" --> K["Generate, print, and measure AprilGrid"]
    K --> L["Capture Allan, stereo, and IMU-camera datasets"]
    L --> Y{"Three-export coherence gate passes?"}
    Y -- "No" --> X
    Y -- "Yes" --> N["Create bags; run Allan and Kalibr externally"]
    N --> O["Structural validation plus manual PDF review"]
    O --> P{"All review gates pass?"}
    P -- "No" --> X
    P -- "Yes" --> Q["Promote serial-specific KALIBR_VERIFIED bundle"]
    Q --> R["Record new VIO dataset"]
    R --> S["Replay with viewer and operator bounds"]
    S --> T{"Replay is physically plausible?"}
    T -- "No" --> X
    T -- "Yes" --> U["Run live viewer"]
```
...
repository-local Ceres 2.1.0, repository-local librealsense 2.56.5, and actual
...
(
  ...
  printf '%s\n' \
    ...
    'Reboot or log out and back in, then resume at README Step 2.'
)
```
...
`sudo` to later README Docker blocks, because it would create root-owned
...
grep -E \
  '^(serial|gyro_requested_rate_hz|gyro_rate_hz|gyro_profile_fallback|accelerometer_requested_rate_hz|accelerometer_rate_hz|accelerometer_profile_fallback|motion_correction_available|motion_correction_active|imu_frame|gyroscope_value_unit|accelerometer_value_unit|accelerometer_axis_policy):' \
  "${IMU_PREFLIGHT_REPORT}"
```

For the checked-in stream config, require the requested and active rates to be
200 Hz gyro and 250 Hz accelerometer, both fallback fields to be `false`,
motion correction to be available and active, and the serial to match. Runtime
selection now rejects a missing exact motion profile instead of silently using
the nearest rate. The librealsense motion API supplies gyro in rad/s and
accelerometer in m/s²; OVRS rotates accelerometer samples into gyro-stream
coordinates and linearly interpolates them to gyro timestamps. No unit scale
multiplier is guessed by the OpenVINS adapter.
...
disagreement, missing staged images, invalid image dimensions/indexes, or
incorrect staged IMU row counts. Continue only after `calibration export set:
PASS`; the report is still
...
The script refuses serial/rate/motion-policy disagreement and also requires
the Allan YAML itself to report `/imu0` at the captured gyro rate. Recording
duration is retained in the preparation manifest but is not a hardcoded
promotion threshold; Allan fit quality remains part of mandatory manual
review.
...
independent multi-orientation intrinsic review. `kalibr_imu.yaml` is emitted as
standard YAML for Kalibr's PyYAML reader; the separate OpenVINS YAML and
preparation manifest retain the repository's OpenCV-compatible directive.
When reviewed multi-orientation matrices are available, add
`--imu-intrinsics-yaml PATH` and
`--acknowledge-multi-orientation-intrinsics-reviewed`; both are required, their
motion-correction policy must match the captures, and the input hash is retained.
...
The pinned Kalibr commit used by this procedure defines `--cams`; the README
does not guess between moving-version CLI variants. Never continue after an
...
behaviour, timestamp-delta gaps, target dimensions, physical transform
direction/baseline, time-offset sign, and Allan fit/inflation.
...
## 12. Record, replay, and run live with verified calibration

Record a fresh dataset after calibration:

```bash
: "${VERIFIED_DIR:?Complete Step 11 promotion first}"
: "${D435I_SERIAL:?Complete Step 11 promotion first}"
VIO_RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)"
export VIO_RUN_ID
VIO_DATASET="datasets/vio_${VIO_RUN_ID}"
...
if ./build/linux-release/ovrs_record \
  ...
  echo "STOP: verified VIO recording failed." >&2
  unset VIO_DATASET
fi
```

Replay first. Do not add `--serial` and do not add the unverified override:

```bash
: "${VIO_RUN_ID:?Run the Step 12 recording block first}"
: "${VIO_DATASET:?Complete the Step 12 recording first}"
: "${VERIFIED_DIR:?Complete Step 11 promotion first}"
REPLAY_RUN="runs/replay_${VIO_RUN_ID}"

if ./build/linux-release/ovrs_replay \
     --dataset "${VIO_DATASET}" \
     --config "${VERIFIED_DIR}/estimator.yaml" \
     --rate 1 \
     --viewer \
     --output "${REPLAY_RUN}"; then
  export REPLAY_RUN
else
  echo "STOP: replay failed." >&2
  unset REPLAY_RUN
fi
```

For a return-to-start test, enter bounds derived from the physical test you
actually performed:
...
if python3 scripts/plot_trajectory.py \
...
Only after replay is plausible, run the live viewer:
...
: "${VERIFIED_DIR:?Complete Step 11 promotion first}"
: "${D435I_SERIAL:?Complete Step 11 promotion first}"
LIVE_RUN="runs/live_${VIO_RUN_ID}"
...
Press `q` or `Esc` in the viewer, or Ctrl+C in the terminal, for clean
shutdown. A headless session cannot display a GUI; save a PNG instead:
