...
### Post-calibration replay addendum
...
| --- | --- | --- |
| Project | `0.5.2` from root `VERSION` | v0.5.2 portable/Python/documentation checks are recorded in the current addendum; full Ubuntu/OpenCV/OpenVINS build remains pending |
| Ubuntu | 24.04 x86-64 primary; 22.04 secondary | Ubuntu 24.04.4 x86-64 preflight passes with one system-librealsense warning; observed kernel 7.0.0 is newer than the v2.56.5 release validation list |
| OpenVINS | tag `v2.7`, commit `93adc241390d13e99232652cf05cbe18a93c7bea` | Exact revision and parser contract inspected; current non-ROS/local-Ceres link/runtime pending Ubuntu rebuild |
...
| librealsense2 | exact `2.56.5`, tag `v2.56.5`, commit `38a41441971387197193ad3aeae3cefe6a11f2cb` | Prior repository-local install/runtime/physical capture verified; current v0.5.2 hardware runtime remains pending |
...
| D435i firmware | read-only; never changed by this repository | Observed 5.17.3.10. The v2.56.5 release supports D400 firmware 5.17.0.10 or later, although 5.17.3.10 was released with SDK 2.58.1; compatibility remains a runtime check, not a reason to flash automatically |
| OpenCV | major version 4; hardware recorder now requires `core`, `imgproc`, `imgcodecs`, `highgui`; OpenVINS also uses `features2d`, `calib3d` | Ubuntu package reports 4.6.0; v0.5.2 preview link/runtime pending |
...
| Python | optional for C++ runtime; required for supported calibration validation/promotion; CPython 3.11-3.13 | Fourteen workflow tests and syntax passed on Windows 3.13.12; Ubuntu Python is 3.12.3 |
| Matplotlib | optional venv pin `3.11.1` | PyPI lists CPython 3.13 support; the repository Ubuntu venv cannot be executed from Windows |
| PyYAML | optional venv pin `6.0.3` | Validator/promotion tests passed with an already available Python 3.13 installation; no global package was installed or modified |
| Tk | Ubuntu `python3-tk`; optional interactive plotting backend | installer includes it; preflight treats absence as an optional-feature warning; Ubuntu rerun pending |
...
- librealsense 2.56.5, whose official release lists Ubuntu 24.04 support:
  <https://github.com/realsenseai/librealsense/releases/tag/v2.56.5>
